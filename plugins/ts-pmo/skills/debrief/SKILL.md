...
- items_ds     (✅ Work Items)
- worklog_ds   (📓 Work Log)
- daily_log    (📅 Daily Log — a **page** with one dated child page per day, not a DB; **7-day live window + monthly archive** — see step 5b)
...
4. WORK LOG upsert (idempotent on **Source URL**). **Source URL** = the conversation's canonical link in the Claude **chat app**; in **Claude Code** (no URL) use a stable session identifier, and if neither exists fall back to matching on **Title + Date** — never write a row with an empty key, which would collide and break dedup. **Locate the existing row with `search`/`fetch`, not SQL** (some Notion plans block SQL queries); dedup on Source URL, or Title + Date for git-derived rows. Find the row with this key → update; else create with Title, **Date = the work's date** (today for a live debrief; the commit/work date when backfilling a past day), Summary, Key decisions, Next actions, Source URL, **Surface** (Claude Code vs Claude chat), **Kind**, the **Effort** relation, and the **Work Item** relation if the work maps to one.
5. DAILY LOG upsert (the human, date-organized view). **For each day in scope that has visible work** — the current chat's day, or any day with Work Log rows — **create its page if missing and refresh it**: fetch the 📅 Daily Log page, enumerate its child pages, find the `YYYY-MM-DD · Weekday` page **or create it**, and write/refresh a readable brief synthesizing that day's Work Log rows: **What I did · Where I left off · Pick up here · Decisions · Sources**. **Creating the missing day-page is the job — never just report that a day has no page.** **An existing day-page is not assumed complete, either:** reconcile it against git + the inbox, and where a source shows work the page omits or mislabels — e.g. a day marked "light" that git shows was a build day — **correct and augment it**, noting the addition (e.g. *build work added via git*) so provenance stays honest. A prior chat-only debrief routinely under-reports build work that lives only in git. Idempotent on the date — re-running a day updates its page, never duplicates. For a day in the window with **no visible work at all** (no current chat, no Work Log rows), don't fabricate a page — record it for the report (step 8) as uncaptured, with the reason. Same work the Work Log holds, organized by **date** instead of by **task**.
5b. RETENTION — **archive, never delete (7-day live window).** After upserting, enforce a 7-day window on the 📅 Daily Log home: move any day-page older than 7 days under a collapsed **`📦 Archive / YYYY-MM`** child (create the month bucket if missing). **Archive, not delete** — the day-page carries synthesis (the git-augmented narrative + "pick up here") that isn't a pure render of Work Log rows, so deleting it loses that; the Work Log stays source of truth for the structured facts. The home then shows ~7 recent days + month archives. Idempotent — an already-archived day stays put, never re-created or duplicated.
