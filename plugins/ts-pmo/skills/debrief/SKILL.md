...
1. SCOPE — resolve the time window first:
   ...
   **What you can actually see — and say so.** Your sources are the **current chat** and the **Notion Work Log** (always), plus **`recent_chats`** *only in the Claude chat app*. **In Claude Code you cannot read past conversations**, so for a window beyond the current chat you capture (a) the current chat, fully, and (b) any **past day that already has Work Log rows**, rebuilt into its Daily Log page — nothing else. **Tell the user this up front**: name what you're logging from, and that older chats aren't readable from here, so it's clear what got captured and what didn't. List the **days in scope**. Every write is idempotent (see Idempotency), so re-running an overlapping or repeated window is always safe — it backfills and refreshes in place, never duplicates.
