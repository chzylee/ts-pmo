...
Sometimes you have a lot thrown onto your plate and keeping track of everything across so many spaces takes mental capacity away from actually doing things. _ts pmo._

**Task Steward · Project Management Ops** — a project manager for Claude Code that runs inside your Notion.

TS PMO turns Claude Code into your project management aide. You teach it what you're working on, ask it to track your work across boards in Notion, and it learns how you work and helps keep you on course. Notion becomes the hub for your work so Claude can stay in sync with you, and you get neat views to look at everything.

![TS PMO effort board screenshot](board-screenshot.png)

If you're using this and start thinking ts pmo, let me know. I want to keep this sharp.
...
3. **Install the skills.** They're Agent Skills, so you don't need the `/plugin` command
   ...
   Then start a new chat so they load. _(On the terminal you can instead run
   `claude plugin marketplace add chzylee/ts-pmo` then `claude plugin install ts-pmo@chzylee/ts-pmo`.)_
...
- **Your git commits** — durable and timestamped, so build work is captured _even from
  sessions you never debriefed_.
...
what matters to _you_ — TS PMO hands that part back, every time.
...
- **Just say `uninstall ts-pmo`.** The `ts-pmo-uninstall` skill inventories everything it put
  on your machine, confirms, then removes it — _including_ the session-end hook in your
